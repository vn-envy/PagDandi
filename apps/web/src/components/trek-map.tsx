        ...
        <div className="relative flex items-center justify-center">
          <span
            className={cn(
              ...
              isSos ? "bg-red-500" : stale ? "bg-muted-foreground" : "bg-foreground",
            )}
          />
          <span
            className={cn(
              "relative size-4 rounded-full border-2 border-background shadow-lg",
              isSos ? "bg-red-500" : stale ? "bg-muted-foreground/60" : "bg-foreground",
            )}
          />
        </div>
      </MarkerContent>
...
function PoiMarker({ poi }: { poi: PoiFeature }) {
  ...
  return (
    <MapMarker longitude={poi.lng} latitude={poi.lat}>
      <MarkerContent>
        <div
          className="flex size-7 items-center justify-center rounded-full border-2 border-white/90 shadow-md"
...
function YouMarker({ position }: { position: Position }) {
  return (
    <MapMarker longitude={position.lng} latitude={position.lat} draggable={false}>
      <MarkerContent>
        <div className="relative flex items-center justify-center">
          <span className="absolute size-10 rounded-full bg-foreground/20 animate-pulse" />
          <span className="relative size-5 rounded-full border-[3px] border-background bg-foreground shadow-xl ring-2 ring-foreground/25" />
        </div>
      </MarkerContent>
    ...
    <div className="relative h-full min-h-[320px] w-full">
      ...
      >
        ...
        <MapRoute
          ...
          color={mapTheme === "light" ? "#171717" : "#fafafa"}
          width={4}
          opacity={0.9}
          interactive={false}
        />
        ...
        <MapMarker longitude={manifest.summit.lng} latitude={manifest.summit.lat}>
          <MarkerContent>
            <Mountain className="size-6 text-foreground drop-shadow" />
          </MarkerContent>
          <MarkerLabel>{manifest.summit.name}</MarkerLabel>
        </MapMarker>
      </Map>
      <div className="pointer-events-none absolute left-3 top-3 z-10 rounded-xl border border-border/60 bg-background/80 px-3 py-2 text-xs backdrop-blur-md">
        <p className="font-semibold">Offline · PMTiles</p>
