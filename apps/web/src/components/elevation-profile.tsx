...
export function ElevationProfile({
  ...
  const chart = (
    ...
    >
      <path d={areaD} className="fill-orange-500/15" />
      <path d={pathD} className="fill-none stroke-orange-500" strokeWidth={2} />

      {points.map((w) => (
        <g key={w.id}>
          <circle cx={w.xy[0]} cy={w.xy[1]} r={3} className="fill-orange-600 stroke-background" strokeWidth={1.5} />
          <text
            x={w.xy[0]}
            y={H - 6}
            textAnchor="middle"
      ...
      <line
        ...
        strokeDasharray="3 2"
      />
      <circle cx={cursor.x} cy={cursor.y} r={5} className="fill-blue-500 stroke-white" strokeWidth={2} />
      <text
        x={cursor.x}
        y={Math.max(10, cursor.y - 9)}
        textAnchor="middle"
        className="fill-blue-500 text-[9px] font-semibold"
      >
        {cursor.elevationM}m
      </text>
    </svg>
